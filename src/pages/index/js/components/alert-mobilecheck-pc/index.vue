...
export default {
  ...
  methods: {
    ...
    login(){
      ...
      axios.post(api.apiURL.pcLogin,datas,{withCredentials:true})
        .then(response=>{
          ...
          if(response.data.res_code==1){
            ...
            this.$emit('closeBox',true, response.data.data.token);
          }else{
              Toast("网络故障，请稍后尝试");
              this.$emit('closeBox',false);
          }
    ...
    sendsms() {
      if(this.isMobileValid()){
        ...
        axios.post(api.apiURL.pcSendsms,str)
          .then(response=> {
              if(response.data.res_code==1){
